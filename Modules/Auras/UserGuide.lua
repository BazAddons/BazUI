    ...
    pages = {
        {
            ...
            blocks = {
                ...
                { type = "paragraph", text = "A row's icons touch when spacing is set to nought - the button is exactly what you can see of it, with nothing hanging outside its box. Each icon is trimmed a little at the edges, because every icon in the game's art has a dark border painted into it that would otherwise sit on top of ours. |cffffd700Icon shape|r on the General page makes them square or rounded." },
                { type = "list", items = {
                    "Right-click an icon on |cffffd700one of your own rows|r to cancel that buff, or to remove a weapon enchant - in combat as well as out of it. A target's buffs are not yours to cancel, so a right-click there does nothing.",
                    "Hover an icon for the aura's own tooltip, the same one the game's own buff frame shows.",
...

        {
            ...
            blocks = {
                ...
                { type = "table", columns = { "Setting", "What it does" }, rows = {
                    ...
                    { "Space beside", "Pixels left between this row and whatever shares its line. One number for the line, so setting it on either of the two is enough." },
                    { "Counts toward stack width", "Off, whatever docks above or below this stack ignores this row when sizing itself." },
...

        {
            ...
            blocks = {
                ...
                { type = "note", text = "Layout changes made during combat apply the moment combat ends. Counts and timers on icons already on screen keep running normally." },
                { type = "note", style = "warning", text = "|cffffd700A fight has a limit on new icons.|r The game will not let an addon make a button while you are fighting, so each row keeps four spare ones ready. Gain more than four new auras in a single fight and the rest appear when it ends. Resizing waits for the same reason, which is why a row can briefly wear two sizes mid-fight." },
            },
        },
    },
})
