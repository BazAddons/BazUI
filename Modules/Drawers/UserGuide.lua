...

        {
            ...
            blocks = {
                { type = "table", columns = { "Widget", "What it shows" }, rows = {
                    ...
                    { "Micro Menu", "The game's own row of micro buttons - character, spellbook, talents, quests, the game menu - as a widget." },
                } },
                ...
                { type = "table", columns = { "Setting", "What it does" }, rows = {
                    ...
                    { "Hide TomTom's coordinates", "Puts away the little block showing where you are standing. TomTom's own setting is left alone - this only holds the frame down, so turning it off gives you the block back exactly as TomTom had it." },
                    { "Zygor Waypoint", "The same for Zygor's arrow." },
                } },

                { type = "h3", text = "The quest timer" },
                { type = "paragraph", text = "Blizzard's timer is a child of Blizzard's tracker, which this widget hides - so a timed quest used to count down where nobody could see it. Scalding Mornbrew Delivery gives you five minutes and no clock, which is the quest failing rather than the interface being untidy." },
                { type = "paragraph", text = "It is rescued onto a frame of its own and left to get on with it: their timer already knows how to count, how to list several at once, and what to do when one runs out. Drag it in |cffffd700Edit Mode|r like anything else, or use |cffffd700Reset position|r on its inspector panel." },
                { type = "note", text = "It is only there while a quest is actually timed. The frame it sits on has a size of its own so there is still something to grab in Edit Mode when no quest is running, which is the one moment you do want to be arranging your interface." },
                { type = "note", text = "Show Blizzard's tracker again and the timer goes back inside it, where their layout expects it." },

                { type = "h3", text = "Micro Menu" },
                { type = "paragraph", text = "The game's micro buttons, as a widget. The Character button shows your portrait instead of a fixed icon." },
                { type = "paragraph", text = "|cffffd700They are still the game's own buttons|r, only moved and redressed. Tooltips, keybinds, the talent reminder flash and the pressed-in look of a window that is already open all keep working, because none of that is being reimplemented." },
                { type = "paragraph", text = "Which buttons exist is the client's answer, not a list written here - and the game decides when one exists at all: Talents before level 10, Guild while you are in none." },
                { type = "table", columns = { "Setting", "What it does" }, rows = {
                    { "Orientation", "A row or a column. Rows become columns when it runs the other way." },
                    { "Rows", "Wrap the buttons over more than one line - ten buttons in two rows of five. The rows are filled evenly." },
                    { "Button size", "22 to 44 pixels." },
                    { "Spacing", "The gap between buttons." },
                    { "BazUI skin", "Round icons in the suite's ring, which is how the rest of the addon draws a button. Off leaves Blizzard's own art alone - the buttons still sit on this widget and still move with it, they simply keep the shape the game drew them. They are not square, so an unskinned row is sized by height and each button keeps its own width." },
                    { "Hide Blizzard's micro menu", "Parks the stock row in the bottom right. Its buttons are on this widget either way." },
                } },
                { type = "paragraph", text = "Below those, one switch per button. Turning one off takes it off the widget and closes the gap; it does not disable whatever it opened, which still has its keybind." },
                { type = "note", text = "|cffffd700Only the buttons this client has get a switch.|r There is no Achievements button on WoW: Forever, so there is no switch for one - a setting you could never act on is clutter, not honesty." },
                { type = "note", text = "The switches are listed in a fixed order. The widget itself follows the order the game gives its buttons, so the two lists can read differently." },
                { type = "note", text = "Turn the |cffffd700Micro Menu module|r off on BazUI > General and the buttons go back to the game's own row in the bottom right." },
            },
        },
...

        {
            ...
            blocks = {
                { type = "table", columns = { "Command", "What it does" }, rows = {
                    ...
                    { "/bazmicro", "Opens this page too - the micro menu is one of these widgets, so its settings are here with the rest." },
                    { "/bazmicro debug", "Prints what the micro menu currently thinks its state is, worth having in a bug report." },
                } },
            },
        },
    },
